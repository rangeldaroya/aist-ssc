...
def get_f_feats(x, x_test, y):
    ...
    FVAL_THRESH = 500
    sorted_fvals = {k: v for k, v in sorted(fvals.items(), key=lambda item: item[1]) if v>FVAL_THRESH}
    f_feats = [k for k,v in sorted_fvals.items()]

    return x[f_feats].values, x_test[f_feats].values


def get_mi_feats(x, x_test, y):
    ...
    mi_feats = [k for k,v in sorted_mi_vals.items()]
...
def get_vif_feats(x, x_test, y, VIF_THRESH = 10):
